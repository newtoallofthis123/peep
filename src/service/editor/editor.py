...
class Editor:
    ...
    def content(self):
        if self.exits():
            with open(os.path.join(os.getcwd(), self.file), "r") as file:
                content = file.read()
                return content
        else:
            return None
    def write(self, content):
        with open(os.path.join(os.getcwd(), self.file), "w") as file:
            file.write(content)
            return True
    ...
    def server(self, lang, content, file_path):
        ...
        def write():
            ...
            self.write(content)
            return f"<h1 style='font-family: consolas, monospace; font-size: 84px; text-align: center;'>🐍 SNAKE RUN <br><br> Saved file {self.file}!</h1>"

        from ..brain import port_assign
        import webbrowser
        port=port_assign()
        webbrowser.open(f'http://localhost:{str(port)}/{self.file}')
...
class Md:
    ...
    def up(self):
        ...
        from ..brain import port_assign
        port=port_assign()
        webbrowser.open(f'http://localhost:{str(port)}/{self.name}')
        app.run(port=port, debug=False)